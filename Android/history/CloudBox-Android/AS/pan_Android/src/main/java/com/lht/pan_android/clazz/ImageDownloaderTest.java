...
public class ImageDownloaderTest {
	 ...
	 */
	public boolean imageDownload(String url, String auth, ImageView mImageView, String path, Activity mActivity,
		...
		String imageName = "";
		if (url != null) {
			imageName = ImageLoaderUtil.getInstance().getImageName(url);
		}
		...
		// 文件中也没有，此时根据mImageView的tag，即url去判断该url对应的task是否已经在执行，如果在执行，本次操作不创建新的线程，否则创建新的线程。
			...
			MyAsyncTask task = new MyAsyncTask(url, auth, mImageView, path, mActivity, download);
			if (mImageView != null) {
				Log.i(TAG, "执行MyAsyncTask --> " + ImageLoaderUtil.flag);
	...
	private class MyAsyncTask extends AsyncTask<String, Void, Bitmap> {
		...
		public MyAsyncTask(String url, String auth, ImageView mImageView, String path, Activity mActivity,
				OnImageDownload download) {
		...
		protected Bitmap doInBackground(String... params) {
			...
			if (url != null && auth != null) {
				try {
					URL c_url = new URL(url + auth);
					InputStream bitmap_data = c_url.openStream();
					data = BitmapFactory.decodeStream(bitmap_data);
					String imageName = ImageLoaderUtil.getInstance().getImageName(url);
					if (!setBitmapToFile(path, mActivity, imageName, data)) {
						removeBitmapFromFile(path, mActivity, imageName);
					}
					// imageCaches.put(url, new
					// SoftReference<Bitmap>(data.createScaledBitmap(data,
					// width, height, true)));
					imageCaches.put(url, new SoftReference<Bitmap>(Bitmap.createBitmap(data)));
				} catch (Exception e) {
					e.printStackTrace();
				}
			}
