...
public class QQShareUtil {
	...
	public static void qShareImage7Text(final QShareImage7TextBean bean, final Activity activity,
		...
		// Title 必须
		String title = StringUtil.isEmpty(bean.getTitle()) ? "创意云盘分享" : bean.getTitle();
		...
		// 返回append 建议
		String from = StringUtil.isEmpty(bean.getFrom()) ? "创意云盘" : bean.getFrom();
	...
	private static void doShareToQQ(final Bundle params, final Activity activity, final IUiListener callback) {
		...
		ThreadManager.getMainHandler().post(new Runnable() {
			...
			@Override
			public void run() {
				if (null != CloudBoxApplication.mTencent) {
					CloudBoxApplication.mTencent.shareToQQ(activity, params, callback);
				} else {
					DLog.e(getClass(), new LogLocation(), "mTencent is null!");
				}
