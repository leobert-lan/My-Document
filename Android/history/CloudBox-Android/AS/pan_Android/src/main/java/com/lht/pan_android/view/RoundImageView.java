...
public class RoundImageView extends ImageView {
	...
	private void setCustomAttributes(AttributeSet attrs) {
		...
		mOutBorderThickness = a.getDimensionPixelSize(R.styleable.roundedimageview_border_outthickness, 0);
		mInnerBorderThickness = a.getDimensionPixelSize(R.styleable.roundedimageview_border_innerthickness, 0);
	...
	protected void onDraw(Canvas canvas) {
		...
		// }
		float length = (defaultWidth < defaultHeight ? defaultWidth : defaultHeight) / 2;
		...
		if (mBorderInsideColor != defaultColor && mBorderOutsideColor != defaultColor) {
			...
			// 画内圆
			drawCircleBorder(canvas, radius + mInnerBorderThickness, mBorderInsideColor, mInnerBorderThickness);
			// 画外圆
			drawCircleBorder(canvas, radius + mBorderThickness, mBorderOutsideColor, mOutBorderThickness);
			...
			drawCircleBorder(canvas, radius + mOutBorderThickness, mBorderOutsideColor, mOutBorderThickness);
		} else {// 没有边框
	 ...
	 * 边缘画圆
	 */
	private void drawCircleBorder(Canvas canvas, float radius, int color, int thick) {
		Paint paint = new Paint();
		/* 去锯齿 */
		paint.setAntiAlias(true);
		paint.setFilterBitmap(true);
