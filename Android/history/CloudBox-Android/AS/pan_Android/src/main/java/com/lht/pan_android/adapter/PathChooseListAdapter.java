...
/**
 * @ClassName: PathChooseListAdapter
 * @Description: 目录列表适配器
 ...
 * @version 1.0
 */
public class PathChooseListAdapter extends AbsUserListAdapter
	...
	public PathChooseListAdapter(ArrayList<DirItemBean> mData, Context mContext) {
		this.mLiData = mData;
	...
	protected void click(int position) {
		...
		if (dirItemBean.getType().equals(IKeyManager.UserFolderList.TYPE_FOLDER)) {
			...
			((PathChooseActivity) mContext).mPathChooseUtil.performOperate(Operate.click_to_access,
					dirItemBean.getName(), dirItemBean.getPath());
			// 但是应该即刻更新目标路径，这时候也允许submit
			((PathChooseActivity) mContext).updateDestination(dirItemBean.getPath());
		}
	}
