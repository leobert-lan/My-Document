...
public class MoveUtil implements IKeyManager.UserFolderList, IKeyManager.Token, IUrlManager.CreateNewFolder {
	...
	public MoveUtil(MoveActivity activity) {
	...
	public IGetList getOperateCallBack() {
		...
		return new IGetList() {
			...
			public void onFinish(ArrayList<DirItemBean> list, DirPaginationBean dirPagination) {
				...
				MoveUtil.this.dirPagination = dirPagination;
				mActivity.getPullRefreshListView().onRefreshComplete();
			}
	 ...
	 * @param foldersNeedToMove
	 * @param pathToMove
	 */
	public void move(ArrayList<DirItemBean> filesNeedToMove, ArrayList<DirItemBean> foldersNeedToMove,
			String pathToMove) {

		Log.d(tag, "path" + pathToMove);

		String url = IUrlManager.Move.DOMAIN + IUrlManager.Move.ADDRESS + username + IUrlManager.Move.FUNCTION;
		url += "?access_token=" + accessToken + "&access_id=" + accessId;

		StringEntity entity = getEntity(filesNeedToMove, foldersNeedToMove, pathToMove);
		httpUtil.postWithEntity(mActivity, url, entity, "application/json", new AsyncHttpResponseHandler() {

			@Override
			public void onSuccess(int arg0, Header[] arg1, byte[] arg2) {
				// TODO 通知操作成功，关闭activity
				mActivity.finish();
			}

			@Override
			public void onFailure(int arg0, Header[] arg1, byte[] arg2, Throwable arg3) {
				DLog.d(MoveUtil.class, new LogLocation(), "移动文件失败，status：" + arg0);
				HttpRequestFailureUtil failureUtil = new HttpRequestFailureUtil(mActivity);
				failureUtil.handleFailureWithCode(arg0, false);
			}
		});
	...
	private StringEntity getEntity(ArrayList<DirItemBean> filesNeedToMove, ArrayList<DirItemBean> foldersNeedToMove,
			String pathToMove) {
		JSONObject jObj = new JSONObject();
		JSONArray jArray = new JSONArray();
		for (int i = 0; i < filesNeedToMove.size(); i++) {
			jArray.add(filesNeedToMove.get(i).getPath());
		}
		for (int i = 0; i < foldersNeedToMove.size(); i++) {
			jArray.add(foldersNeedToMove.get(i).getPath());
		}
		jObj.put(IUrlManager.Move.KEY_ABSOLUTEPATH, jArray);
		jObj.put(IUrlManager.Move.KEY_MOVETO, pathToMove);
		StringEntity ret = null;
		try {
			ret = new StringEntity(jObj.toJSONString(), HTTP.UTF_8);
		} catch (UnsupportedEncodingException e) {
