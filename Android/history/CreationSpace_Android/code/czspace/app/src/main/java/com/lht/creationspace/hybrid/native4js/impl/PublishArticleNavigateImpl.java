...
public class PublishArticleNavigateImpl extends ABSApiImpl
        implements Native4JsExpandAPI.PublishArticleNavigateHandler {

    public PublishArticleNavigateImpl(Context ctx) {
        super(ctx);
    }
...
//            } else {
            ArticlePublishActivity.getLauncher(mContext).launch();
            if (callBackFunction != null)
...
//            return BEAN_IS_ERROR;
    ...
    public static BridgeNativeFunction newInstance(Context context) {
        return new BridgeNativeFunction(API_NAME, new PublishArticleNavigateImpl(context));
    }
}
