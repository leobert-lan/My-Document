 ...
 * <p><b>Classname</b> NestedScrollLayout
 * <p><b>Description</b>: an layout which contains a header section and a scrollable section like
 * ListView,ScrollView and so on.
 * When we scroll,if header is shown,firstly change this layout to hide the header and expand the
 * scrollable section,then the scrollable section consume the moving event of finger,while the
 * header is hidden,scroll section always consume the moving event of finger only when the scroll
 * arrive the top and scroll down continues,which will drag the header to display;
 *
 * by the way,the scrollable section may not be a scroll things, just most time we annoy because
 * it is a scrollable section. So, an AttachUtil is provided;
...
public class NestedScrollLayout extends FrameLayout {
    ...
    private void init(AttributeSet attrs) {
        ...
        // init from attrs
        TypedArray a = getContext().obtainStyledAttributes(attrs, R.styleable.NestedScrollLayout);
        setCollapseOffset(a.getDimensionPixelSize(R.styleable
                .NestedScrollLayout_nslCollapseOffset, collapseOffset));
        overDrag = a.getBoolean(R.styleable.NestedScrollLayout_nslOverDrag, overDrag);
        contentViewId = a.getResourceId(R.styleable.NestedScrollLayout_nslContentView, -1);
        headerViewId = a.getResourceId(R.styleable.NestedScrollLayout_nslHeaderView, -1);
        initOpen(a.getBoolean(R.styleable.NestedScrollLayout_nslOpen, true));
        captureTop = a.getBoolean(R.styleable.NestedScrollLayout_nslCaptureTop, true);
        a.recycle();
    }
    ...
    public void onFinishInflate() {
        ...
        if (headerViewId != -1 && contentViewId == -1) {
            throw new IllegalArgumentException("You have set \"nslHeaderVIew\" but miss " +
                    "\"nslContentView\". Both are required,or default to be selected by the " +
                    "order of declaration!");
        }

        if (contentViewId != -1 && headerViewId == -1) {
            throw new IllegalArgumentException("You have set \"nslContentView\" but miss " +
                    "\"nslHeaderVIew\". Both are required,or default to be selected by the " +
                    "order of declaration!");
        }
    ...
    private void bindId(View view) {
        topView = view.findViewById(headerViewId);
        dragContentView = view.findViewById(contentViewId);

        if (topView == null) {
            throw new IllegalArgumentException("\"dtlTopView\" with id = \"@id/"
                    + getResources().getResourceEntryName(headerViewId)
                    + "\" has NOT been found. Is a child with that id in this " + getClass()
                    .getSimpleName() + "?");
        }
        ...
        if (dragContentView == null) {
            throw new IllegalArgumentException("\"dtlDragContentView\" with id = \"@id/"
                    + getResources().getResourceEntryName(contentViewId)
    ...
    public NestedScrollLayout setTouchMode(boolean shouldIntercept) {
     ...
     * @return SetupWizard
     */
    public NestedScrollLayout setRefreshRatio(float ratio) {
     ...
     * @return SetupWizard
     */
    public NestedScrollLayout setOverDrag(boolean overDrag) {
     ...
     * @return
     */
    public NestedScrollLayout setcontentViewId(int id) {
     ...
     * @return
     */
    public NestedScrollLayout setheaderViewId(int id) {
        this.headerViewId = id;
        return this;
    }
     ...
     * @return SetupWizard
     */
    public NestedScrollLayout setCollapseOffset(int px) {
        collapseOffset = px;
        resetContentHeight();
        return this;
    }
