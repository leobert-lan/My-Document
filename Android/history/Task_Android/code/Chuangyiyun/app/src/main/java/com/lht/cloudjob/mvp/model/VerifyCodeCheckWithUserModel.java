...
public class VerifyCodeCheckWithUserModel implements IApiRequestModel {
    ...
    private IRestfulApi.CheckVerifyCodeWithUserApi api;

    private RequestHandle handle;

    public VerifyCodeCheckWithUserModel(String username, String code,
                                        ApiModelCallback<BaseVsoApiResBean> callback) {
        modelCallback = callback;
        mHttpUtil = HttpUtil.getInstance();
        api = new IRestfulApi.CheckVerifyCodeWithUserApi();
        params = api.newRequestParams(username, code);
    }
