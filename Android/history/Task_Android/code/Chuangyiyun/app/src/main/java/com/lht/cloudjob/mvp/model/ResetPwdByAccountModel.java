 ...
 * <p><b>Classname</b> ResetPwdByAccountModel
 * <p><b>Description</b>: 使用账户重置密码，一般使用phone
 * Created by leobert on 2016/7/15.
 * <p>
 * to see API at{@link IRestfulApi.ResetPwdByAccountApi}
 * to see ResBean at{@link BaseVsoApiResBean}
 */
public class ResetPwdByAccountModel implements IApiRequestModel {

    private final ApiModelCallback<BaseVsoApiResBean> modelCallback;

    private final HttpUtil mHttpUtil;

    private IRestfulApi.ResetPwdByAccountApi api;

    private RequestParams params;

    private RequestHandle handle;

    public ResetPwdByAccountModel(String account, String password, ApiModelCallback<BaseVsoApiResBean> callback) {
        this.modelCallback = callback;
        this.mHttpUtil = HttpUtil.getInstance();
        api = new IRestfulApi.ResetPwdByAccountApi();
        params = api.newRequestParams(account, password);
    }
